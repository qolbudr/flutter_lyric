...
class LyricPainter extends CustomPainter with ChangeNotifier {
  ...
  //老的上一行位置
  static int _oldCurrentLyricIndex = 0;
  ...
  LyricPainter(this.lyrics,
      ...
      : this.lyricTextStyle = lyricStyle ?? TextStyle(color: Colors.grey, fontSize: 13),
    ...
    lyricTextPaints.addAll(lyrics
        .map(
          (l) => TextPainter(textAlign: TextAlign.start, text: TextSpan(text: l.lyric, style: lyricTextStyle), textDirection: TextDirection.ltr),
        )
        .toList());

    //翻译/音译歌词转画笔
    if (subLyrics != null && (subLyrics ?? []).isNotEmpty) {
      subLyricTextPaints.addAll(subLyrics!.map((l) => TextPainter(textAlign: TextAlign.start, text: TextSpan(text: l.lyric, style: subLyricTextStyle), textDirection: TextDirection.ltr)).toList());
      //因空行高度与非空行高度不一致，先存一个非空行的位置
      notEmptySubLyricIndex = getNotEmptyLineHeight(subLyrics!);
      //计算默认歌词高度
      subLyricTextPaints[notEmptySubLyricIndex]..layout(maxWidth: lyricMaxWidth);
  ...
  int getNotEmptyLineHeight(List<Lyric> lyrics) => lyrics.indexOf(lyrics.firstWhere((lyric) => lyric.lyric.trim().isNotEmpty, orElse: () => lyrics.first));
  ...
  animationScrollY(currentLyricIndex, TickerProvider tickerProvider) {
    if (currentLyricIndex != 0 && currentLyricIndex != _oldCurrentLyricIndex) {
      ...
      _animationController = AnimationController(vsync: tickerProvider, duration: Duration(milliseconds: 200))
      ...
      // 起始为上一行，结束点为当前行
      Animation animation = Tween<double>(begin: previousRowOffset, end: currentRowOffset).animate(_animationController!);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    var currentLyricY = _offset + lyricTextPaints[currentLyricIndex].height / 6;
    ...
    for (int lyricIndex = 0; lyricIndex < lyrics.length; lyricIndex++) {
      ...
      if (currentLyricY < size.height && currentLyricY > 0) {
        ...
        currentLyricTextPaint
          //设置歌词
          ..text = TextSpan(text: currentLyric.lyric, style: currentLyricIndex == lyricIndex ? currLyricTextStyle : lyricTextStyle)
          //计算文本宽高
          ..layout(maxWidth: lyricMaxWidth)
          //绘制 offset=横向居中
          ..paint(canvas, Offset(0, currentLyricY));
      }
      ...
      if (subLyrics != null) {
        List<Lyric> remarkLyrics = subLyrics!.where((subLyric) => (subLyric.startTime! >= currentLyric.startTime!) && subLyric.endTime! <= currentLyric.endTime!).toList();
        remarkLyrics.forEach((remarkLyric) {
          ...
          if (currentLyricY < size.height && currentLyricY > 0) {
            subLyricTextPaints[subIndex] //设置歌词
              ..text = TextSpan(text: remarkLyric.lyric, style: subLyricTextStyle)
              //计算文本宽高
              ..layout(maxWidth: lyricMaxWidth)
              //绘制 offset=横向居中
              ..paint(canvas, Offset(0, currentLyricY));
  ...
  int findLyricIndexByDuration(double curDuration, List<Lyric> lyrics) {
    for (int i = 0; i < lyrics.length; i++) {
      if (curDuration >= (lyrics[i].startTime?.inMilliseconds ?? 0) && curDuration <= (lyrics[i].endTime?.inMilliseconds ?? 0)) {
        return i;
      }
    }
    return 0;
  ...
  double computeScrollY(int curLine) {
    ...
    if (subLyrics != null) {
      //增加 当前行之前的翻译歌词的偏移量
      var list = subLyrics!.where((subLyric) => subLyric.endTime! <= lyrics[curLine].endTime!).toList();
